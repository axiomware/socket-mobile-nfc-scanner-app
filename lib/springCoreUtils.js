...
class springCoreUtils {
    ...
    parseFrame(hexStrData, node = nodeIDdefault){//pass indication with device ID. Indication will be accumalated till '00'
        if(this._isHexStr(hexStrData)) {
            ...
            let pObj = this._parseHeader(node)

            if (!pObj)
                return;
            
            let header = pObj.header
            let payload = pObj.payload
            let dobj = this._parse(payload, header['len']);
    ...
    {
        let newData = Buffer.from(hexStr, 'hex')
        if(node in this._indicationData){
    ...
    _getTagInfoDetails(tagInfoHexStr) {
        if(!this._isHexStr(tagInfoHexStr)) 
            throw new Error(`Input data is not hex string`)
        let tagInfoBuf = Buffer.from(tagInfoHexStr, 'hex')

        if (tagInfoBuf.byteLength != 4)
            throw new Error(`tag Info is not 4 bytes[${tagInfoBuf.byteLength}]`)

        let ifmap = interfaceIDMap.get(tagInfoBuf[1])
        if(!ifmap)
            throw new Error(`tag Info does not have Interface ID`)
        let protocolmap = ifmap['protocolMap']
        return {
            new: (tagInfoBuf[0] & 0x80)?true:false,
            dataUTF8: (tagInfoBuf[0] & 0x20)?true:false,
            detailsUTF8: (tagInfoBuf[0] & 0x10)?true:false,
    ...
    _parseHeader(node = nodeIDdefault) {
        ...
        if(this._isHeaderLong(pcb)){
            if(data.byteLength < 6)
                throw new Error(`Header too short[${data.byteLength}]`);
...
//Unknown protocol
let protocol00Map = new Map([
    [0x00, 	'Unknown protocol']
])

//NFC/RFID HF
let protocol03Map = new Map([
...
//NFC/RFID UHF
let protocol06Map = new Map([
    [0x03, 	'ISO/IEC 18000-6C / EPC Class 1 Gen2 UHF']
])

//Bluetooth interface
let protocol84Map = new Map([
