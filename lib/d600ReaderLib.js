...
class d600Utils {
     ...
     * @param  {string} node Use this tag to keep track of partial data from multiple sources
     */
    parseFrame(hexStrData, node = nodeIDdefaul){//pass indication with device ID. Indication will be accumalated till '00'
        if(this._isHexStr(hexStrData)) {
            this._addData(hexStrData, node)

            if(this._EOF(node)){
                let hexStr = this._getData(node)
                let cardTypeStr = Buffer.from(hexStr.slice(0, 4), 'hex').toString('utf8')
                this.clearData(node)
    ...
    decode(parsedFrame){
        ...
        if(parsedFrame['header']){
            ...
            let cType = parseInt(CtStr, 16);//first two chars denote card type or url
    ...
    {
        if((node in this._indicationData) && (this._indicationData[node].slice(-2) == '00'))
            return true
        else 
            return false
    }
